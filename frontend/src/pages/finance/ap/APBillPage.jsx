/**
 * APBillPage —— 应付单（finance-gl 应付款管理，doc_type ACCOUNTS_PAYABLE，完全替代金蝶应付单）
 *
 * 落 UX 律 14：台账(Table over /api/query) → 录入抽屉(不跳页) → 动作按钮(走 /api/transition 唯一写入路径)。
 *   头：供应商 F7 / 币别 / 汇率 / 业务日期 / 到期日 / 立账类型(业务应付·暂估应付) / 付款条件 / 采购员 F7 / 采购组织 /
 *       开关「价外税」「按含税单价录入」「先到票后入库」。本位币双金额 base_amount = amount × exchange_rate（前端实时算，提交带上）。
 *   明细网格(ap_bill_line)：物料 F7 / 计价数量 / 单价 / 税率组(带出税率) / 不含税 / 税额 / 价税合计 —— 按头开关实时算：
 *       价外税：不含税=数量×单价；税额=不含税×税率%；价税合计=不含税+税额。
 *       价内税(含税单价)：价税合计=数量×单价；不含税=价税合计/(1+税率%)；税额=价税合计−不含税。
 *   付款计划子表(ap_payment_plan_line)：到期日 / 比例% / 计划金额（手填，比例合计应=100）。
 *   状态机 DRAFT 暂存 → SUBMITTED 提交 → AUDITED 审核（审核后业财映射生应付凭证，Phase2 effect）。
 *
...
const DOC_TYPE = 'ACCOUNTS_PAYABLE';
const TABLE = 'accounts_payable';
const LINE_TABLE = 'ap_bill_line';
const LINE_FK = 'accounts_payable_id';
const PLAN_TABLE = 'ap_payment_plan_line';
const PLAN_FK = 'accounts_payable_id';

const EDITABLE_STATES = new Set(['DRAFT', '']); // '' = 尚未建单（新建录入态）

const CURRENCIES = ['USD', 'HKD', 'CNY', 'EUR'];
const BILL_TYPE_OPTS = [
  { label: '业务应付', value: 'BUSINESS_AP' },
  { label: '暂估应付', value: 'PROVISIONAL_AP' },
];
...
const BILL_STATUS = {
  ...
  PENDING: { label: '待付(旧)', color: 'gold' },
  PARTIAL: { label: '部分付款(旧)', color: 'blue' },
  PAID: { label: '已付清(旧)', color: 'green' },
};
...
const newPlan = (over = {}) => ({
  ...
  due_date: null, ratio: 0, plan_amount: 0, paid_amount: 0, remark: '',
  ...over,
});
...
export default function APBillPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const aps = data?.data || [];
      const { data: sd } = await query('supplier', { limit: 1000 });
      const suppById = new Map((sd?.data || []).map((s) => [s.id, s]));
      setRows(aps.map((ap) => {
        const s = suppById.get(ap.supplier_id) || {};
        return { ...ap, _supplier: s.short_name || s.name || (ap.supplier_id ? `供应商#${ap.supplier_id}` : '—') };
      }));
    } catch (e) {
      message.error('应付单台账加载失败：' + (e.response?.data?.detail || e.message));
      setRows([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      setSuppliers(await loadFkOptions('supplier', 'supplier_id'));
      setMaterials(await loadFkOptions('material', 'material_id'));
      setPurchasers(await loadFkOptions('user_account', 'purchaser_id'));
      setCompanies(await loadFkOptions('company', 'purchase_org_id'));
    })();
  }, []);
  ...
  const openDetail = useCallback((row, edit = false) => {
    ...
    form.setFieldsValue({
      ...
      bill_type: row?.bill_type || 'BUSINESS_AP',
      ...
      purchaser_id: row?.purchaser_id ?? null,
      purchase_org_id: row?.purchase_org_id ?? null,
      settle_org_id: row?.settle_org_id ?? null,
      purchase_dept: row?.purchase_dept || '',
      is_tax_included: row?.is_tax_included ?? true,
      is_price_tax_inclusive: !!row?.is_price_tax_inclusive,
      is_goods_first: !!row?.is_goods_first,
  ...
  const openNew = useCallback(() => {
    ...
    form.resetFields();
    form.setFieldsValue({
      currency: 'USD', exchange_rate: 1, bill_type: 'BUSINESS_AP',
      bill_date: dayjs(), is_tax_included: true, is_price_tax_inclusive: false, is_goods_first: false,
    });
    setOpen(true);
  }, [form]);
  ...
  // 保存（建/改）。
  //   新建：引擎建单 doc_id=null 入 START（同事务取应付单号），再 START→DRAFT 推进落「暂存」可编辑态。
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      ...
      || String(r._supplier || '').toLowerCase().includes(q));
  }, [rows, kw]);

  const columns = [
    { title: '应付单号', dataIndex: 'bill_number', width: 150, fixed: 'left', render: (v, r) => v ? <span style={{ fontFamily: MONO }}>{v}</span> : <span style={{ color: '#bfbbb5', fontFamily: MONO }}>#{r.id}</span> },
    { title: '供应商', dataIndex: '_supplier', width: 180 },
  ...
  const planColumns = [
    ...
    { title: '已付', dataIndex: 'paid_amount', width: 110, align: 'right', render: (v) => <span style={{ fontFamily: MONO }}>{fmtMoney(v)}</span> },
    {
      title: '', width: 44, fixed: 'right',
      render: (_, __, i) => editable ? (
        <Button type="text" size="small" danger icon={<DeleteOutlined />}
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
        <h2 style={{ fontSize: 26, fontWeight: 300, letterSpacing: '-0.01em', color: '#000', margin: 0, lineHeight: 1.2 }}>应付单</h2>
        <span style={{ color: '#777169', fontSize: 13 }}>
          财务 / 应付款管理 · 供应商债务立账（完全替代金蝶应付单）· 账簿 = {user?.company_name || `公司 #${user?.company_id ?? ''}`}
        </span>
      </div>

      <Card size="small" style={{ borderRadius: 14 }} styles={{ body: { padding: 0 } }}>
        <div style={{ display: 'flex', gap: 8, padding: 12, alignItems: 'center' }}>
          <Button type="primary" icon={<PlusOutlined />} onClick={openNew}>新建应付单</Button>
          <span style={{ flex: 1 }} />
          <Input.Search allowClear placeholder="应付单号 / 供应商" value={kw} onChange={(e) => setKw(e.target.value)} style={{ width: 240 }} />
          <Button icon={<ReloadOutlined />} loading={loading} onClick={load}>刷新</Button>
        </div>
        {!loading && !rows.length ? (
          <Empty style={{ padding: 40 }} description="暂无应付单，点「新建应付单」录入" />
        ) : (
      ...
      <Drawer
        ...
        title={`应付单 · ${editMode ? (detail?.id ? '编辑' : '新建') : '详情'}${detail?.bill_number ? ` · ${detail.bill_number}` : ''}`}
        extra={
          <Space>
      ...
      >
        <Form form={form} layout="vertical" disabled={!editable}>
          <Row gutter={16}>
            <Col span={8}><Form.Item name="supplier_id" label="供应商" rules={[{ required: true, message: '请选供应商' }]}>
              <Select showSearch optionFilterProp="label" placeholder="供应商 F7" options={suppliers} /></Form.Item></Col>
          ...
          <Row gutter={16}>
            ...
            <Col span={6}><Form.Item name="payment_terms_text" label="付款条件"><Input placeholder="如 Net 30 / T/T 预付" /></Form.Item></Col>
            <Col span={5}><Form.Item name="purchaser_id" label="采购员">
              <Select showSearch optionFilterProp="label" allowClear placeholder="采购员 F7" options={purchasers} /></Form.Item></Col>
            <Col span={5}><Form.Item name="purchase_org_id" label="采购组织">
              <Select showSearch optionFilterProp="label" allowClear placeholder="采购组织" options={companies} /></Form.Item></Col>
            <Col span={4}><Form.Item name="purchase_dept" label="采购部门"><Input /></Form.Item></Col>
          </Row>
          <Row gutter={16}>
            <Col span={5}><Form.Item name="settle_org_id" label="结算组织">
              <Select showSearch optionFilterProp="label" allowClear placeholder="结算组织" options={companies} /></Form.Item></Col>
            <Col span={6}><Form.Item name="invoice_number" label="发票号"><Input placeholder="供应商发票号" /></Form.Item></Col>
          </Row>
          <Row gutter={16} align="middle">
            ...
            <Col span={5}><Form.Item name="is_goods_first" label="先到票后入库" valuePropName="checked">
              <Switch checkedChildren="先票后货" unCheckedChildren="先货后票" /></Form.Item></Col>
            <Col span={8}><Form.Item name="remark" label="备注"><Input /></Form.Item></Col>
          </Row>
        </Form>
        ...
        <Tabs
          items={[
            ...
            {
              key: 'plan', label: `付款计划（${planRows.filter((r) => !r._delete).length}）`,
              children: (
                <>
                  {editable && (
                    <Button size="small" type="dashed" icon={<PlusOutlined />} style={{ marginBottom: 8 }}
                      onClick={() => setPlanRows((prev) => [...prev, newPlan()])}>添加付款计划行</Button>
                  )}
