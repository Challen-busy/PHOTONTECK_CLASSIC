/**
 * APPaymentPage —— 付款单（finance-gl 应付款管理，doc_type AP_PAYMENT，完全替代金蝶付款单）
 *
 * 落 UX 律 14：台账(Table over /api/query) → 录入抽屉(不跳页) → 动作按钮(走 /api/transition 唯一写入路径)。
 *   头：供应商 F7 / 币别 / 汇率 / 付款日期 / 结算方式 F7 / 银行账户 / 收款方 / 付款用途 / 付款金额 / 开关「是否预付」/ 备注。
 *       本位币双金额 base_amount = amount × exchange_rate（前端实时算，提交带上）。
 *   状态机 DRAFT 暂存 → AUDITED 审核（审核后业财映射生凭证：借 2202 应付<冲减>或 1123 预付<is_advance> / 贷 1002 银行，
...
const DOC_TYPE = 'AP_PAYMENT';
const TABLE = 'ap_payment';
...
export default function APPaymentPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const supById = new Map((sd?.data || []).map((s) => [s.id, s]));
      setRows(recs.map((r) => {
        const s = supById.get(r.supplier_id) || {};
        return { ...r, _supplier: s.short_name || s.name || (r.supplier_id ? `供应商#${r.supplier_id}` : '—') };
      }));
    } catch (e) {
      message.error('付款单台账加载失败：' + (e.response?.data?.detail || e.message));
      setRows([]);
  ...
  const openDetail = useCallback((row, edit = false) => {
    ...
    form.setFieldsValue({
      ...
      payment_date: row?.payment_date ? dayjs(row.payment_date) : dayjs(),
  ...
  const openNew = useCallback(() => {
    ...
    form.resetFields();
    form.setFieldsValue({ currency: 'USD', exchange_rate: 1, payment_date: dayjs(), amount: 0, is_advance: false });
    setOpen(true);
  }, [form]);
  ...
  const columns = [
    { title: '付款单号', dataIndex: 'payment_number', width: 150, fixed: 'left', render: (v, r) => v ? <span style={{ fontFamily: MONO }}>{v}</span> : <span style={{ color: '#bfbbb5', fontFamily: MONO }}>#{r.id}</span> },
    { title: '供应商', dataIndex: '_supplier', width: 180 },
    { title: '付款日期', dataIndex: 'payment_date', width: 110, render: (v) => v || <span style={{ color: '#bfbbb5' }}>—</span> },
    { title: '付款金额', dataIndex: 'amount', width: 130, align: 'right', render: money },
    { title: '币种', dataIndex: 'currency', width: 64 },
    { title: '本位币', dataIndex: 'base_amount', width: 130, align: 'right', render: money },
    { title: '预付', dataIndex: 'is_advance', width: 70, render: (v) => v ? <Tag color="purple">预付</Tag> : <span style={{ color: '#bfbbb5' }}>—</span> },
    ...
    },
    {
      ...
      render: (v) => { const s = PAYMENT_STATUS[v] || { label: statusLabel(v), color: 'default' }; return <Tag color={s.color}>{s.label}</Tag>; },
    },
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
        <h2 style={{ fontSize: 26, fontWeight: 300, letterSpacing: '-0.01em', color: '#000', margin: 0, lineHeight: 1.2 }}>付款单</h2>
        <span style={{ color: '#777169', fontSize: 13 }}>
          财务 / 应付款管理 · 供应商实付款登记（完全替代金蝶付款单）· 账簿 = {user?.company_name || `公司 #${user?.company_id ?? ''}`}
        </span>
      </div>

      <Card size="small" style={{ borderRadius: 14 }} styles={{ body: { padding: 0 } }}>
        <div style={{ display: 'flex', gap: 8, padding: 12, alignItems: 'center' }}>
          <Button type="primary" icon={<PlusOutlined />} onClick={openNew}>新建付款单</Button>
          <span style={{ flex: 1 }} />
          <Input.Search allowClear placeholder="付款单号 / 供应商" value={kw} onChange={(e) => setKw(e.target.value)} style={{ width: 240 }} />
          <Button icon={<ReloadOutlined />} loading={loading} onClick={load}>刷新</Button>
        </div>
        {!loading && !rows.length ? (
          <Empty style={{ padding: 40 }} description="暂无付款单，点「新建付款单」录入" />
        ) : (
      ...
      <Drawer
        ...
        title={`付款单 · ${editMode ? (detail?.id ? '编辑' : '新建') : '详情'}${detail?.payment_number ? ` · ${detail.payment_number}` : ''}`}
        extra={
          <Space>
            {detail?.id && <Tag color={(PAYMENT_STATUS[status] || {}).color}>{(PAYMENT_STATUS[status] || {}).label || statusLabel(status)}</Tag>}
            {docActions.map((a) => (
      ...
      >
        <Form form={form} layout="vertical" disabled={!editable}>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="supplier_id" label="供应商" rules={[{ required: true, message: '请选供应商' }]}>
          ...
          <Row gutter={16}>
            <Col span={8}><Form.Item name="payment_date" label="付款日期"><DatePicker style={{ width: '100%' }} /></Form.Item></Col>
            ...
            <Col span={8}><Form.Item name="bank_account" label="银行账户"><Input placeholder="付款银行账户" /></Form.Item></Col>
          </Row>
          <Row gutter={16}>
            <Col span={8}><Form.Item name="payee_name" label="收款方"><Input /></Form.Item></Col>
            <Col span={8}><Form.Item name="payment_purpose" label="付款用途"><Input placeholder="付款用途" /></Form.Item></Col>
            <Col span={8}><Form.Item name="amount" label="付款金额" rules={[{ required: true, message: '请填付款金额' }]}>
              <InputNumber style={{ width: '100%' }} min={0} precision={2} onChange={(v) => setAmount(num(v))} /></Form.Item></Col>
          </Row>
          <Row gutter={16}>
            <Col span={8}><Form.Item name="is_advance" label="是否预付" valuePropName="checked">
              <Switch checkedChildren="预付" unCheckedChildren="非预付" /></Form.Item></Col>
          </Row>
          <Form.Item name="remark" label="备注"><Input.TextArea rows={2} /></Form.Item>
        </Form>

        <Row gutter={24} style={{ marginTop: 8 }}>
          <Col><Statistic title="付款金额（原币）" value={fmtMoney(amount)} valueStyle={{ fontFamily: MONO, fontSize: 18, color: '#389e0d' }} /></Col>
