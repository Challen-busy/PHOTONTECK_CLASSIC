/**
 * APWriteoffPage —— 应付核销工作台（finance-gl·应付款管理，owns by C·核销界面 PM）
 *
 * 落「债务↔已付勾稽」：左栏待核销应付单（我方债务侧）/ 右栏待核销付款·预付（已付侧），多对多核销。
 *   · 顶部过滤：供应商(F7) / 币别 / 结算组织 / 核销方案，拉 /api/reports/ap-open-items 出两侧未清明细。
 ...
 *
 * 通用件：biz_type 参数化（本页固定 AP），与应收核销页共用同一骨架（换 biz_type=AP + 标签）。
...
import { query, getApOpenItems, writeoff, unwriteoff } from '../../../api';
import { MONO, fmtMoney, statusLabel } from '../financeHelpers';

// 本次核销状态标签（应付单 writeoff_status / 付款单 writeoff_status 取值）。
const WO_STATUS = {
...
export default function APWriteoffPage() {
  const { user } = useAuth();
  ...
  const [debitItems, setDebitItems] = useState([]);   // 待核销应付（我方债务）
  const [creditItems, setCreditItems] = useState([]); // 待核销付款/预付（已付）
  const [debitSel, setDebitSel] = useState([]);       // 勾选的应付单 id
  const [creditSel, setCreditSel] = useState([]);     // 勾选的付款单 id
  ...
  const suppName = useCallback((id) => {
    const c = suppById.get(id);
    return c ? (c.short_name || c.name || `供应商#${id}`) : (id ? `供应商#${id}` : '—');
  }, [suppById]);

  // 过滤条主数据（供应商 F7 / 结算组织（公司）/ 核销方案）
  useEffect(() => {
    (async () => {
      try {
        const [{ data: supp }, { data: comp }, { data: sch }] = await Promise.all([
          query('supplier', { limit: 1000, order_by: 'code' }),
          query('company', { limit: 100, order_by: 'id' }),
          query('writeoff_scheme', { filters: { biz_type: 'AP', is_active: true }, order_by: 'priority', limit: 100 }),
        ]);
  ...
  const loadOpen = useCallback(async () => {
    setLoading(true);
    try {
      const params = { biz_type: 'AP' };
      if (partyId) params.party_id = partyId;
      if (currency) params.currency = currency;
      const { data } = await getApOpenItems(params);
  ...
  // 已核销关系（通用 writeoff_link，biz_type=AP + is_active）
  const loadLinks = useCallback(async () => {
    setLinksLoading(true);
    try {
      const { data } = await query('writeoff_link', { filters: { biz_type: 'AP', is_active: true }, order_by: '-id', limit: 300 });
      setLinks(data?.data || []);
      setLinkSel([]);
    } catch (e) {
  ...
  // 手工核销：左右各取 1（多对多由后端逐对消化；本页一次提交一组「单应付×单付款」或一笔均摊）
  ...
  const doManualWriteoff = async () => {
    ...
    setSubmitting(true);
    try {
      const payload = { biz_type: 'AP', auto: false, links: linksPayload };
  ...
  const doAutoWriteoff = async () => {
    ...
    setSubmitting(true);
    try {
      const payload = { biz_type: 'AP', auto: true, scheme_id: schemeId };
      if (partyId) payload.party_id = partyId;
      if (currency) payload.currency = currency;
      if (settlementOrgId) payload.settlement_org_id = settlementOrgId;
      const { data } = await writeoff(payload);
      if (!data?.created) message.info('未配对到可核销组合（方案规则下无匹配债务/已付）');
  ...
  // ── 左栏：待核销应付（我方债务）列 ──
  const debitColumns = [
    { title: '应付单号', dataIndex: 'number', width: 150, fixed: 'left', render: (v, r) => <span style={{ fontFamily: MONO }}>{v || `#${r.id}`}</span> },
    { title: '供应商', dataIndex: 'party_id', width: 130, render: (v) => suppName(v) },
  ...
  // ── 右栏：待核销付款/预付（已付）列 ──
  const creditColumns = [
    { title: '付款单号', dataIndex: 'number', width: 150, fixed: 'left', render: (v, r) => <span style={{ fontFamily: MONO }}>{v || `#${r.id}`}</span> },
    { title: '供应商', dataIndex: 'party_id', width: 130, render: (v) => suppName(v) },
    { title: '付款日期', dataIndex: 'payment_date', width: 100, render: (v) => v || dash },
    { title: '付款金额', dataIndex: 'amount', width: 110, align: 'right', render: money },
    { title: '已核销', dataIndex: 'written_off_amount', width: 100, align: 'right', render: money },
    { title: '未核销', dataIndex: 'open_amount', width: 110, align: 'right', render: (v) => <strong style={{ fontFamily: MONO }}>{fmtMoney(v)}</strong> },
    { title: '币别', dataIndex: 'currency', width: 56 },
    { title: '状态', dataIndex: 'writeoff_status', width: 90, render: woTag },
  ...
  const linkColumns = [
    ...
    { title: '应付单', dataIndex: 'debit_doc_id', width: 150, render: (v, r) => <span style={{ fontFamily: MONO }}>{r.debit_doc_type} #{v}</span> },
    { title: '付款单', dataIndex: 'credit_doc_id', width: 150, render: (v, r) => <span style={{ fontFamily: MONO }}>{r.credit_doc_type} #{v}</span> },
    ...
    <div>
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
        ...
        <span style={{ color: '#777169', fontSize: 13 }}>
          财务 / 应付款管理 · 债务（应付单）↔ 已付（付款/预付）多对多勾稽 · 账簿 = {user?.company_name || `公司 #${user?.company_id ?? ''}`}
        </span>
      </div>
        ...
        <Space wrap size={16} align="end">
          <Fld label="供应商（F7）">
            <Select size="small" style={{ width: 200 }} allowClear showSearch optionFilterProp="label"
              placeholder="全部供应商" value={partyId ?? undefined} onChange={setPartyId}
              options={suppliers.map((c) => ({ value: c.id, label: `${c.short_name || c.name}${c.code ? `（${c.code}）` : ''}` }))} />
          </Fld>
          <Fld label="币别">
      ...
      <Row gutter={14}>
        <ACol span={12}>
          <Card size="small" title={<span style={{ fontWeight: 600 }}>待核销应付（债务侧）</span>}
            ...
            {loaded && !debitItems.length ? <Empty style={{ padding: 32 }} description="无未核销应付单" /> : (
        ...
        <ACol span={12}>
          <Card size="small" title={<span style={{ fontWeight: 600 }}>待核销付款 / 预付（已付侧）</span>}
            extra={<span style={{ fontSize: 12, color: '#777169' }}>{creditItems.length} 张 · 已选 {creditSel.length}</span>}
            style={{ borderRadius: 14 }} styles={{ body: { padding: 0 } }}>
            {loaded && !creditItems.length ? <Empty style={{ padding: 32 }} description="无未核销付款单" /> : (
      ...
      <Card size="small" style={{ borderRadius: 14, marginTop: 14 }}>
        <Row gutter={24} align="middle">
          <ACol><Statistic title="应付侧本次核销额" value={fmtMoney(sums.debit)} valueStyle={{ fontFamily: MONO, fontSize: 20 }} /></ACol>
          <ACol><Statistic title="已付侧本次核销额" value={fmtMoney(sums.credit)} valueStyle={{ fontFamily: MONO, fontSize: 20 }} /></ACol>
          <ACol>
            <Statistic title="差额（应付−已付）" value={fmtMoney(sums.diff)}
