...
import {
  Card, Tabs, Table, Button, Modal, Form, Input, Select, Space, message, Spin,
} from 'antd';
...
function UsersTab() {
  ...
  return (<>
    ...
    <Card style={{ borderRadius: 16, boxShadow: CARD_SHADOW, border: 'none' }} styles={{ body: { padding: 0 } }}>
      <Table
        dataSource={users}
        rowKey="id"
        size="small"
        ...
        columns={[
          ...
          { title: '角色', dataIndex: 'role', render: v => <Pill bg="#f5f2ef" color="#4e4e4e">{v}</Pill> },
          { title: '公司ID', dataIndex: 'company_id', width: 80 },
          {
            title: '管理员', dataIndex: 'is_admin', width: 80,
            render: v => v ? <Pill bg="#fdecea" color="#b42318">是</Pill> : <span style={{ color: '#bfbbb5' }}>—</span>,
          },
          {
            title: '状态', dataIndex: 'is_active', width: 80,
            render: v => v
              ? <Pill bg="#ebf5ee" color="#1f8f3a">启用</Pill>
              : <Pill bg="#f5f5f5" color="#4e4e4e">停用</Pill>,
          },
        ]}
      />
    </Card>
        ...
        <Card
          ...
          title={(
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontWeight: 500, letterSpacing: '0.01em' }}>{wf.name}</span>
              <span style={{ color: '#777169', fontSize: 12, fontFamily: 'ui-monospace, monospace' }}>v{wf.version}</span>
              {wf.is_frozen && <Pill bg="#fdecea" color="#b42318">冻结</Pill>}
            </div>
          )}
          size="small"
          style={{ marginBottom: 16, borderRadius: 16, boxShadow: CARD_SHADOW, border: 'none' }}
        >
          {wf.description && (
            <p style={{ color: '#4e4e4e', marginBottom: 12, letterSpacing: '0.01em' }}>{wf.description}</p>
          )}
          <Table
            dataSource={(wf.states || []).map((s, i) => ({ ...s, _idx: i }))}
            rowKey="code"
            size="small"
            pagination={false}
            columns={[
              {
                ...
                render: (v, r) => (
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    <strong style={{ fontWeight: 500, color: '#000' }}>{v}</strong>
                    {r.is_initial && <Pill bg="#fbf5e4" color="#b8860b">起</Pill>}
                    {r.is_terminal && <Pill bg="#f5f2ef" color="#4e4e4e">终</Pill>}
                  </span>
                ),
              },
              { title: '角色', dataIndex: 'allowed_roles', render: v => v?.join(', ') || <span style={{ color: '#bfbbb5' }}>全部</span> },
              {
                title: '用户能填的项', dataIndex: 'editable_fields',
                render: v => v?.length ? v.join(', ') : <span style={{ color: '#bfbbb5' }}>无（仅推进按钮）</span>,
              },
              {
                title: '可推进到', dataIndex: 'next',
                render: v => v?.length
                  ? v.map(n => `${n.label} → ${codeToName[n.to] || n.to}`).join('；')
                  : <span style={{ color: '#bfbbb5' }}>终止</span>,
              },
              {
                title: '自动校验规则', dataIndex: 'hard_rules',
                render: v => v?.length
                  ? <span style={{ fontSize: 11, fontFamily: 'ui-monospace, monospace' }}>{v.join('；')}</span>
                  : <span style={{ color: '#bfbbb5' }}>无</span>,
              },
              {
                title: '描述', dataIndex: 'description', ellipsis: true, width: 200,
                render: v => v || <span style={{ color: '#bfbbb5' }}>无</span>,
              },
              {
    ...
    <Modal title={`编辑节点: ${editing?.state?.name || ''}`} open={!!editing} onCancel={() => setEditing(null)} onOk={saveState} width={700}>
      <Form form={form} layout="vertical">
        ...
        <Form.Item name="hard_rules" label="自动校验规则（判定式 DSL，每行一条）"
          ...
          <Input.TextArea
            rows={5}
            placeholder="一行一条表达式。留空 = 没有自动校验，靠 Agent 看描述判断"
...
function KnowledgeTab() {
  ...
  const typeStyle = {
    RULE:  { bg: '#eaf1fb', color: '#1f5aa8' },
    ALERT: { bg: '#fbf5e4', color: '#b8860b' },
    GUIDE: { bg: '#ebf5ee', color: '#1f8f3a' },
    FAQ:   { bg: '#f5f2ef', color: '#4e4e4e' },
  };
        ...
        columns={[
          {
            ...
            render: v => {
              const s = typeStyle[v] || typeStyle.FAQ;
              return <Pill bg={s.bg} color={s.color}>{v}</Pill>;
            },
          },
    ...
    <Modal title="新增知识库条目" open={modal} onCancel={() => setModal(false)} onOk={create}>
      <Form form={form} layout="vertical">
        <Form.Item name="entry_type" label="类型" rules={[{ required: true }]}>
          <Select options={[
            { value: 'RULE', label: '业务规则' }, { value: 'ALERT', label: '预警规则' },
            { value: 'GUIDE', label: '操作指南' }, { value: 'FAQ', label: 'FAQ' },
          ]} />
        </Form.Item>
        <Form.Item name="title" label="标题" rules={[{ required: true }]}><Input /></Form.Item>
        <Form.Item name="content" label="内容" rules={[{ required: true }]}><Input.TextArea rows={4} /></Form.Item>
        <Form.Item name="applicable_doc_types" label="适用单据类型(逗号分隔)">
          <Input placeholder="如: SALES_ORDER,PURCHASE_ORDER" />
        </Form.Item>
      </Form>
    </Modal>
  </>);
...
function AlertsTab() {
  ...
  useEffect(() => {
    api.get('/alerts').then(r => { setAlerts(r.data); setLoading(false); }).catch(() => setLoading(false));
  }, []);

  const levelStyle = {
    HIGH:   { bg: '#fdecea', color: '#b42318' },
    MEDIUM: { bg: '#fbf5e4', color: '#b8860b' },
    LOW:    { bg: '#eaf1fb', color: '#1f5aa8' },
  };
        ...
        columns={[
          {
            ...
            render: v => {
              const s = levelStyle[v] || levelStyle.LOW;
              return <Pill bg={s.bg} color={s.color}>{v}</Pill>;
            },
          },
...
function AdminAgentTab() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 260px)' }}>
      <Card
        style={{ flex: 1, overflow: 'auto', marginBottom: 12, borderRadius: 16, boxShadow: CARD_SHADOW, border: 'none' }}
        styles={{ body: { padding: 16 } }}
      >
        {messages.length === 0 && (
          <div style={{ textAlign: 'center', padding: 60, color: '#777169' }}>
            <div style={{
              width: 64, height: 64, margin: '0 auto 16px', borderRadius: 18,
              background: '#f5f2ef', display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <RobotOutlined style={{ fontSize: 32, color: '#4e4e4e' }} />
            </div>
            <div style={{ fontSize: 16, fontWeight: 300, color: '#000', marginBottom: 4 }}>
              流程配置助手
            </div>
            <p style={{ fontSize: 12, margin: 0, letterSpacing: '0.01em' }}>
              试试: "查看所有流程" / "给销售订单加一个总监审批节点" / "列出所有数据表"
            </p>
          </div>
        )}
        {messages.map((m, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{
                ...
                fontSize: 13, letterSpacing: '0.01em', lineHeight: 1.5,
              }}>
                <div style={{ whiteSpace: 'pre-wrap' }}>{m.content}</div>
                {m.tools?.length > 0 && (
                  <div style={{
                    marginTop: 6, fontSize: 10, color: isUser ? 'rgba(255,255,255,0.6)' : '#777169',
                    borderTop: `1px solid ${isUser ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.05)'}`,
                    paddingTop: 4,
                  }}>
  ...
  return (
    <div>
      <h2 style={{
        ...
        color: '#000', margin: '0 0 20px', lineHeight: 1.15,
      }}>
        系统管理
      </h2>
      <Tabs
